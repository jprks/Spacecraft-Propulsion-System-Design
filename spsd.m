...
%% Exit Conditions
exit_mach = solve_mach(exit_area,throat_area,specific_heat_ratio); % Excutes the newton-raphson approximation function to find exit mach
...
exit_pressure(1) = chamber_pressure(1)/(temperature_ratio)^(specific_heat_ratio/(specific_heat_ratio-1));

pressure_ratio = exit_pressure(1)/chamber_pressure(1);

%% System Performance

effective_exhaust_velocity = exhaust_velocity+exit_pressure(1)*exit_area/mass_flowrate_I;
...
thrust_force_I(1) = thrust_coefficient(1)*chamber_pressure(1)*throat_area;

thrust_force_II = effective_exhaust_velocity*mass_flowrate_I;

thrust_force_III = mass_flowrate_I*exhaust_velocity+exit_pressure(1)*exit_area;

specific_impulse_I = thrust_force_I(1)*standard_gravity/mass_flowrate_I;
...
burn_time = propellant_mass(1)/mass_flowrate_I;
...
switch chamber_geometry % finds the volume of the selected tank [Does not work for toroidal tank, 1/4/17]
    case 1
        geometry = 'Spherical';
        radius = 0.05;
        spherical_volume = propellant_mass(1)*specific_gas_constant*chamber_temperature/chamber_pressure(1);
    ...
    case 2
        ...
        height = 0.1;
        width = 0.05;
        rectangular_volume = propellant_mass(1)*specific_gas_constant*chamber_temperature/chamber_pressure(1);
        volume = rectangular_volume;
    case 3
        geometry = 'Cylindrical';
        height = 0.1;
        radius = 0.05;
        cylindrical_volume = propellant_mass(1)*specific_gas_constant*chamber_temperature/chamber_pressure(1);
        volume = cylindrical_volume;
    case 4
        geometry = 'Toroidal';
        minor_radius = 1;
        major_radius = 1;
        torus_volume = propellant_mass(1)*specific_gas_constant*chamber_temperature/chamber_pressure(1);
        volume = torus_volume;
end
...
lim = 1*10^3; % Pressure limit [10000 Pa]

while chamber_pressure >= lim
    dm = mass_flowrate_I*dt; % Calculates the ejected mass per unit dt
    propellant_mass(i+1) = propellant_mass(i) - dm; % Updates the amount of propellant in tank 
    chamber_pressure(i+1) = propellant_mass(i+1)*specific_gas_constant*chamber_temperature/volume; % Finds the new chamber pressure
    exit_pressure(i+1) = chamber_pressure(i+1)/((temperature_ratio)^(specific_heat_ratio/(specific_heat_ratio-1))); % Finds the new exit pressure
    thrust_coefficient(i+1) = sqrt(2*specific_heat_ratio^2/(specific_heat_ratio-1))*(2/(specific_heat_ratio+1))^((specific_heat_ratio+1)/(specific_heat_ratio-1))...
        *(1-(exit_pressure(i+1)/chamber_pressure(i+1))^((specific_heat_ratio-1)/specific_heat_ratio))+(exit_pressure(i+1)/chamber_pressure(i+1))*(area_ratio); % Finds the updated thrust coefficient
    thrust_force_I(i+1) = thrust_coefficient(i+1)*chamber_pressure(i+1)*throat_area; % Calculates the thrust using the updated values
    acceleration(i+1) = thrust_force_I(i+1)*dt*1/dm; % Solving the F = m*a for a
    velocity(i+1) = acceleration(i+1)*dt; % Attempt to iteratively find the system velocity [Does not seem to be working, 1/4/17]
    ...
    impulse_bit(i) = thrust_force_I(i+1)*dt; % Finds the impulse per unit dt
    velocity_bit(i) = velocity(i+1)*dt; % Finds the velocity per unit dt
    
    i = i + 1; % Increases the step count
    t(i+1) = t(1) + dt; % Updates the simulation time
end
...
result = {'Propellant','',PropellantName;
    ...
    'Thrust Force I',thrust_force_I(1),force_unit;
    'Thrust Force II',thrust_force_II(1),force_unit;
...
exceldata = {'Variable','Magnitude','Unit';'Propellant',PropellantName,unitless;'Specific Gas Constant',specific_gas_constant,gas_constant_unit;...
    ...
    'Exit Conditions',linedivider,linedivider;'Exit Temperature',exit_temperature,temperature_unit;'Exit Pressure',exit_pressure(1),pressure_unit;...
    ...
    'Mass Flowrate III',mass_flowrate_III,massflow_unit;'Propellant Mass',propellant_mass(1),mass_unit;'Temperature Ratio',temperature_ratio,unitless;...
    'Pressure Ratio',pressure_ratio,unitless;'Thrust Coefficient',thrust_coefficient(1),unitless;'Thrust Force I',thrust_force_I(1),force_unit;...
